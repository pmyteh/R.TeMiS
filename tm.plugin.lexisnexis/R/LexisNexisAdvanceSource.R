LexisNexisAdvanceSource <- function(x, encoding = "UTF-8") {
    # LexisNexis Advance files have a different format, and HTML output is no longer available :-/
    # textreadr should handle any of the full-text document formats, though: .pdf, .docx, and .rtf.
    lines <- textreadr::read_docx(x)

    # Document starts: First line, plus (by pushing the vector forward by one and trimming the end)
    # the lines following "End of Document"
