...
parseDateAndEdition <- function(s, tid, language=c("en", "fr", "de")) {
  # Parse date from date-plus-edition string.
  #
  # The core problem with LN date parsing is that the date string is localised
  # to the language of the article, while all the field codes (including
  # the LOAD-DATE:) are localised to the language of the computer operating
  # the LN website. Consequently, although we can rely on the other fields being
  ...
  if (getOption("LNUseDateparser", default=FALSE)) {
    ...
    # Don't create a timezone (fairly useless given
    # these are only days anyway); don't return
    ...
    if (!is.null(tup)) {
      ...
      warning("Could not parse document date/edition string for ",
              tid, ", even using the dateparser library: ", s, "\n")
      dt <- NA
      edition <- character(0)
    }
    return(list(date=dt, edition=edition))
